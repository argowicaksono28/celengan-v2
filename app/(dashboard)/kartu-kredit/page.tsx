  ...
  return (
    <div className="bg-white rounded-card border border-c-border p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-xs text-[#64748B] mb-1 block">Limit</label>
          <div className="flex items-center border border-c-border rounded-btn px-2 py-2 gap-1 focus-within:border-primary">
        ...
        <div>
          <label className="text-xs text-[#64748B] mb-1 block">Tagihan saat ini</label>
          <div className="flex items-center border border-c-border rounded-btn px-2 py-2 gap-1 focus-within:border-primary">
            <span className="text-xs text-[#64748B]">Rp</span>
            <input type="text" inputMode="numeric"
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-xs text-[#64748B] mb-1 block">Tgl Billing</label>
        ...
        <div>
          <label className="text-xs text-[#64748B] mb-1 block">Tgl Jatuh Tempo</label>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-[#0F172A]">Kartu Kredit</h1>
        <button
      ...
      {cards.length > 0 && (
        <div className="bg-white rounded-card border border-c-border shadow-card p-4">
          <div className="flex justify-between mb-2">
            <span className="text-sm font-medium text-[#0F172A]">Total Tagihan</span>
          ...
          <p className="text-xs text-[#64748B] mt-1">Limit total: {formatRupiah(totalLimit)}</p>
        </div>
      )}
      ...
      {loading ? (
        <div className="space-y-3">{[1,2].map((i) => <div key={i} className="h-24 bg-white rounded-card border border-c-border animate-pulse" />)}</div>
                ...
                <div className="bg-white rounded-card border border-c-border shadow-card p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      {card.bank && <p className="text-xs text-[#64748B]">{card.bank}</p>}
                    </div>
                    <div className="flex gap-1">
                      <button onClick={() => setEditingId(card.id)} className="p-1.5 rounded-full hover:bg-surface-2 text-[#64748B]"><Edit2 size={13} /></button>
                      <button onClick={() => handleDelete(card.id)} className="p-1.5 rounded-full hover:bg-danger-light hover:text-danger text-[#64748B]"><Trash2 size={13} /></button>
                    </div>
                  </div>
